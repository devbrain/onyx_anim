#!/usr/bin/env bash
# Decode a Commodore CDXL file with both onyx_anim's anim_to_ppm and ffmpeg's
# libavcodec/cdxl.c, then bit-compare every frame.
#
# Usage:
#   cdxl_cross_check.sh <input.cdxl> <anim_to_ppm_path> [<ffmpeg_path>]

set -euo pipefail
if [[ $# -lt 2 ]]; then
    echo "usage: $0 <input.cdxl> <anim_to_ppm> [<ffmpeg>]" >&2
    exit 3
fi
