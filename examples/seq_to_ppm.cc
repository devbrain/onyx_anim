// seq_to_ppm — decode an Atari ST .SEQ animation through the public onyx_anim
// API and write each frame as a binary PPM (P6) into an output directory.
//
// Used by scripts/atari_seq_cross_check.sh to compare against Werner
// Randelshofer's reference Java decoder.
...
bool write_ppm(const std::filesystem::path& path,
               ...
               std::span<const std::uint8_t> palette /* RGB triplets */) {
    ...
    for (std::size_t y = 0; y < height; ++y) {
        for (std::size_t x = 0; x < width; ++x) {
            std::size_t idx = indexed[y * width + x];
...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        std::fprintf(stderr, "Usage: %s <input.seq> <output_dir>\n", argv[0]);
        return EXIT_FAILURE;
    }
