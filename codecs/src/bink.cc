        ...
        class bink_decoder_impl final : public anim_decoder {
            public:
                ...
                audio_track(unsigned int index) const noexcept override {
                    ...
                    // Distinguish DCT vs RDFT for the engine's benefit.
                    if (!header_.audio.empty() && header_.audio[0].use_dct) {
                        t.codec_name = "Bink Audio (DCT)";
                    } else {
                        t.codec_name = "Bink Audio (RDFT)";
                    }
                    return t;
                }
                ...
                take_audio_track(unsigned int index,
                    ...
                    if (!audio_pcm_int8_ || index != 0u || audio_taken_) {
